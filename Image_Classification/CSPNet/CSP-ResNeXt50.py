...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, cardinality, group_width, stride=1):
        ...
        self.conv3 = BN_Conv2d_Leaky(self.out_channels, self.out_channels, 1, 1, 0)
...
class Stem(nn.Module):
    def __init__(self, in_channels, num_blocks, cardinality, group_width, stride=2):
        ...
        self.trans_part0 = nn.Sequential(
            BN_Conv2d_Leaky(self.c0, self.hidden_channels, 1, 1, 0),  #
            nn.AvgPool2d(stride),
        )
    ...
    def __make_block(self, num_blocks, in_channels, cardinality, group_width, stride):
        strides = [stride] + [1] * (num_blocks - 1)  # [2,1,1,1...]  # 1, 1, 1
...
class CSP_ResNeXt(nn.Module):
    def __init__(self, num_blocks, cadinality, group_width, num_classes):
        ...
        self.stem0 = Stem(cadinality * group_width, num_blocks[0], cadinality, group_width, stride=1)  # 256, 3, 32, 4
        self.stem1 = Stem(cadinality * group_width * 2, num_blocks[1], cadinality, group_width*2)  # 512, 4, 32, 8
        self.stem2 = Stem(cadinality * group_width * 4, num_blocks[2], cadinality, group_width*4)  # 1024, 6, 32, 16
        self.stem3 = Stem(cadinality * group_width * 8, num_blocks[3], cadinality, group_width*8)  # 2048, 3, 32, 32
...
def csp_resnext_50_32x4d():
    return CSP_ResNeXt([3, 4, 6, 3], cadinality=32, group_width=4, num_classes=1000)
