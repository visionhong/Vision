...
ANNO_DIR = 'E:\\Computer Vision\\data\\WiderPerson\\Annotations'    # corner bbox (not center)
IMG_DIR = 'E:\\Computer Vision\\data\\WiderPerson\\Images'
...
def main():
    ...
    idxs = random.sample(range(len(annos)), 4)  # 랜덤 인덱스 4개
    new_image, new_annos = update_image_and_anno(img_paths,
                                                  annos,
                                                  idxs,
                                                  OUTPUT_SIZE,
                                                  SCALE_RANGE,
    ...
    for anno in new_annos:
        ...
        cv2.rectangle(new_image, start_point, end_point, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    for anno_file in glob.glob(os.path.join(anno_dir, '*.txt')):
        ...
        with open(anno_file, 'r') as f:
            ...
            for _ in range(num_of_objs):
                ...
                if 3 < obj[0]:  # label이 3보다 큰것은 쓰지 않음
                    continue
